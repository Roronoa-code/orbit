...
(async()=>{
 ...
 try{
  for(const name of process.argv.slice(2).length?process.argv.slice(2):['ux-motion','focus-music'])for(const width of [390,320]){
   ...
   try{
    ...
    await page.locator(width===320?'#narrow':'#wide').click();await page.locator('#run').click();
    await page.waitForFunction(()=>{const text=document.querySelector('#result').textContent;return text.includes('"status": "PASS"')||/Error:|FAIL:/.test(text)},{},{timeout:60000});
