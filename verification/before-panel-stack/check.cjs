...
vm.runInContext(script+`;globalThis.model={set:(d,n,k='steps')=>{selected=d;days=n;metric=k},windowRows,movement,week,dayOffset,anchor,restart,render,extras,pause:p=>{paused=p},hide:h=>{document.hidden=h}}`,sandbox);
...
console.log('PASS: 4 metrics × 30 dates × 3 periods; coherent hourly, sleep and macro totals; prior periods; reference averages; one animation loop; pause, reduced motion and hidden-page stop; embedded font.');
