...
touchSwipe(520,600);assert.equal(m.getExpanded(),false,'finger-up must commit downward swipe');
...
const morph=Array.from(m.pillMorphFrames({left:300,top:70,width:36,height:36},{left:40,top:250,width:340,height:250}));
assert.equal(morph.length,36);assert.equal(morph[0].opacity,0);assert.equal(morph.at(-1).opacity,1);assert.equal(morph.at(-1).offset,1);assert(morph.every(f=>!f.transform.includes('NaN')));
...
assert(!html.includes('type="date"'));assert(html.includes('max-height:65dvh'));
