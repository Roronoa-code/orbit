...
const Health = (() => {
  ...
  function render(){
    ...
    countDots?.stop();countDots=null;WorkoutFocus.settle();MusicPlayer.stop();
    body.innerHTML=page==='body'?bodyView():page==='overview'?overview():page==='sleep'?sleepView():countdown!==null?`<div class="workout-countdown"><p class="countdown-heading">${setup.kind}</p><div class="countdown-play"><canvas id="countdown-dots" aria-hidden="true"></canvas><output class="sr-only" id="workout-count" aria-live="polite">${countdown}</output></div><p class="countdown-invitation">Touch the dots</p><div class="countdown-stages" aria-hidden="true">${[3,2,1].map(n=>`<i data-count-stage="${n}" class="${countdown<=n?'lit':''}"></i>`).join('')}</div><button data-cancel-countdown>Cancel</button></div>`:store.active?sessionView():setup?setupView():workoutHome();
    if(page==='workouts'&&countdown!==null)countDots=HeroDots.mount(q('#countdown-dots'),countdown);
    if(page==='workouts'&&store.active){paintTimer();if(timerFocused)MusicPlayer.mount(q('.workout-music'))}
  ...
  function close(){
    ...
    MusicPlayer.stop();WorkoutFocus.settle();page=null;SurfaceMotion.dismiss(q('#health-page'),()=>{
