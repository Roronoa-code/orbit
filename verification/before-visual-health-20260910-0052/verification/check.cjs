...
const closeTarget={closest:s=>s==='button,a,summary'?nodes.get('#stack-close'):null};
deckNode.fire('touchstart',{touches:[finger(340,300)],target:closeTarget});deckNode.fire('touchend',{changedTouches:[finger(340,300)]});
nodes.get('#stack-close').fire('click',{detail:1});assert.equal(m.getExpanded(),false,'A fresh close-button tap after a swipe must not be swallowed');
...
for(const activity of ['body','workouts','oxygen'])for(const date of m.dateOptions){m.set(date,1);const data=m.Health.sample(activity,date);assert.equal(data.stats.length,3);assert(data.stats.every(row=>Number.isFinite(Number(row[1]))))}
for(const page of ['body','workouts','oxygen']){assert(m.Health.open(page));assert.equal(m.Health.page,page);assert.equal(nodes.get('#health-page').hidden,false);assert(m.closeInline());assert.equal(m.Health.page,null);assert.equal(nodes.get('#health-page').hidden,true)}
