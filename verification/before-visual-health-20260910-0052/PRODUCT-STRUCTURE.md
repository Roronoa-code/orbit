...
The three launcher circles are favourite destinations: Body composition, Workouts and Blood oxygen. Each opens a scrolling page. As the library grows, add a clearly labelled “All health” entry alongside these favourites, rather than compressing every category into the launcher or adding more orb gestures.

Each health page should follow the same reading order: selected value → supporting measurements → trend → full history → individual record. Details can expose source, time, quality and corrections. Sparse measurements must remain sparse, rather than inventing daily values.
...
Samsung describes its Now Bar as appearing while a supported app is active, expanding on tap, and allowing another active item to be reached by swiping up. Orbit borrows this ongoing-activity model inside the app; this is not a Samsung lock-screen integration. [Samsung Now Bar guide](https://www.samsung.com/uk/support/mobile-devices/how-to-use-the-now-bar-on-the-lock-screen-of-your-samsung-galaxy-device/)
...
Working here: orb period taps and metric swipes; expanding cards; inline utility controls; three health pages with demonstration trends/history; a persistent duration-only workout timer with pause/resume/finish; and a live bar reflecting that timer session. Timer records stay in this copy. GPS/Watch sensor capture, phone-to-Watch workout commands, real history integration and replacement of the native app remain separate work.
