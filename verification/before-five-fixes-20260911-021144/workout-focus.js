...
const MusicPlayer=(()=>{
  ...
  function retireArtwork(){
    ...
    document.querySelector('#health-page').classList.remove('music-focus');root?.classList.remove('has-art');backdrop('');
    if(!old)return;if(document.hidden){old.remove();return}
    old.style.opacity='0';setTimeout(()=>old.remove(),460);
  }
  // The blurred backdrop cannot show more detail than its blur radius; a small copy keeps its look
  // without uploading the full-resolution cover a second time.
  function thumbnail(img){try{const width=48,height=Math.max(1,Math.round(width*img.naturalHeight/img.naturalWidth)),canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;canvas.getContext('2d').drawImage(img,0,0,width,height);return canvas.toDataURL('image/jpeg',.82)}catch{return null}}
  function artwork(value){
    ...
    img.decode().then(()=>{
      ...
      const small=thumbnail(img)||value,previous=cover.lastElementChild,layer=document.createElement('div');layer.className='music-art-layer';layer.style.setProperty('--art',`url("${small}")`);layer.append(img);cover.append(layer);
      backdrop(small);page.classList.add('music-focus');node.classList.add('has-art');
  ...
  function stop(animateExit=false){
    ...
    if(animateExit&&node&&!node.hidden&&!document.hidden){
