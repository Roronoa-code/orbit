...
const Health = (() => {
  ...
  function render(){
    ...
    body.innerHTML=page==='body'?bodyView():page==='overview'?overview():countdown!==null?`<div class="workout-countdown"><p>Get ready</p><output class="countdown-number" id="workout-count">${countdown}</output><button data-cancel-countdown>Cancel</button></div>`:store.active?sessionView():setup?setupView():workoutHome();
  ...
  function startCountdown(kind,targetMs,source=q('.workout-primary')){
    ...
    cancelCountdown();setup={kind,targetMs};SurfaceMotion.change(()=>{countdown=3;render()},source);
    countTimer=setInterval(()=>{
      countdown--;
      if(countdown<=0)SurfaceMotion.change(()=>{cancelCountdown();if(action('start',kind,targetMs)){setup=null;render();q('#session-time').focus({preventScroll:true});return true}render();return false},q('#workout-count'));
      else q('#workout-count').textContent=countdown;
    },1000);return true;
  }
