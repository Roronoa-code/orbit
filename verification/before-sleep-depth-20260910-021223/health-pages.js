...
const Health = (() => {
  ...
  function bodyView(){
    ...
    const [label,unit]=bodyFields[bodyMetric],data=sample('body',bodyDate),first=sample('body',rows[0])[bodyMetric],delta=data[bodyMetric]-first,points=bodyPlot(rows),point=points[rows.indexOf(bodyDate)];
  ...
  function startCountdown(kind,targetMs,source=q('.workout-primary')){
    if(store.active||!kinds.includes(kind)||!validTarget(targetMs))return false;
    cancelCountdown();setup={kind,targetMs};timerMode='elapsed';timerFocused=false;SurfaceMotion.change(()=>{countdown=3;render()},source);
    countTimer=setInterval(()=>{
      countdown--;
      if(countdown<=0)SurfaceMotion.change(()=>{cancelCountdown();if(action('start',kind,targetMs)){setup=null;render();q('#session-time').focus({preventScroll:true});return true}render();return false},q('.countdown-play'));
  ...
  function open(which,source=q('#live-bar')){
    ...
    if(page)return SurfaceMotion.change(enter,source);
    enter();SurfaceMotion.reveal(q('#health-page'),source);return true;
  }
  function close(){
    if(!page)return false;
    if(countdown!==null){SurfaceMotion.change(()=>{cancelCountdown();render()},null,()=>q('.workout-primary'));return true}
    if(setup){const kind=setup.kind;SurfaceMotion.change(()=>{setup=null;render();q('#health-scroll').scrollTop=0},null,()=>q('[data-setup="'+kind+'"]'));return true}
    page=null;SurfaceMotion.dismiss(q('#health-page'),q('#live-bar'),()=>{
  ...
  function init(config){
    ...
    q('#health-content').addEventListener('click',event=>{
      ...
      if(b.dataset.session){
        const finish=b.dataset.session==='finish',kind=store.active?.kind;
        const saved=finish?SurfaceMotion.change(()=>action('finish'),b,()=>q('[data-setup="'+kind+'"]')):action(b.dataset.session);
      ...
      else if(b.dataset.setup)SurfaceMotion.change(()=>{setup={kind:b.dataset.setup,targetMs:0};render();q('#health-scroll').scrollTop=0;q('input[name="target"]:checked').focus({preventScroll:true})},b);
