...
// A rapid reopen cancels dismissal; reduced motion completes without leaving a hidden/inert page.
...
reduced.matches=true;let reducedClosed=false;surfaces.reveal(panel);surfaces.dismiss(panel,()=>reducedClosed=true);assert(reducedClosed);assert.equal(surfaces.active,0);
// Countdown dots must respond to touch, return to the number, and stop on disposal/reduced motion.
...
dotController.set(2);advanceDots(64);reduced.matches=true;reduced.fire('change');assert.equal(frames.size,0,'Reduced motion must stop the dot loop');
...
const wideClock=dots.mount(dotCanvas,'24:59:59',true);assert(dotsDrawn.length>100&&dotsDrawn.every(([x,y])=>x>0&&x<390&&y>0&&y<116),'A long focused clock must fit the wide canvas');wideClock.stop();
