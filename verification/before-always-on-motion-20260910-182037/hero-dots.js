...
const HeroDots=(()=>{
  ...
  function mount(canvas,value,wide=false){
    ...
    const size=wide?390:300,height=wide?116:300,dpr=Math.min(2,window.devicePixelRatio||1),particles=[];
    ...
    function set(next){
      ...
      particles.forEach((p,i)=>{const to=shape.points[i];p.visible=to?1:0;p.tx=to?start+to.x*spacing:size/2+Math.cos(i*2.399)*size*.44;p.ty=to?top+to.y*spacing:height/2+Math.sin(i*2.399)*height*.44;if(reduced.matches){p.x.value=p.tx;p.y.value=p.ty;p.alpha=p.visible;p.x.velocity=p.y.velocity=0}});
    ...
    set(value);
    return {set,stop(){stopped=true;cancelAnimationFrame(frame);document.removeEventListener('visibilitychange',visibility);reduced.removeEventListener('change',motionPreference);for(const [type,fn] of Object.entries(events))canvas.removeEventListener(type,fn)}};
  }
