...
for(const width of [390,320])for(const populated of [true,false]){
 ...
 await nav('overview');assert.equal(await p.locator('.health-reading').count(),6);assert.equal(await p.locator('.health-destinations').count(),0);assert.match(await p.locator('[data-home-metric=intake]').innerText(),populated?/550/:/—/);await shot('health');
 ...
 await p.locator('.source-link').tap();await settle();await p.locator('#health-scroll').evaluate(n=>n.scrollTop=0);await p.emulateMedia({reducedMotion:'reduce',forcedColors:'active'});await p.keyboard.press('Tab');await p.locator('#profile-name').focus();assert.notEqual(await p.locator('#profile-name').evaluate(n=>getComputedStyle(n).outlineStyle),'none');await shot('contrast');await p.emulateMedia({reducedMotion:'no-preference',forcedColors:'none'});
