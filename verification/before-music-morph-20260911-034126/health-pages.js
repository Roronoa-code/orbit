...
const Health = (() => {
  ...
  function focusChrome(){const button=q('#health-minimize'),show=page==='workouts'&&workoutRecord===null&&Boolean(store.active)&&timerFocused;if(button.hidden===!show)return;button.hidden=!show;if(show)SurfaceMotion.reveal(button,{x:0,y:-6},240)}
  function setFocus(value){const live=q('.workout-live');if(!live||timerFocused===value)return;timerFocused=value;WorkoutFocus.toggle(live,value);focusChrome()}
