...
const WorkoutFocus=(()=>{
  ...
  function dispose(){finish();settle();dots?.stop();dots=null}
  ...
  /* One progress value p moves the workout view (0) and the page-wide music view (1) into each other, so a
     ...
     the dial ring and its labels fade. Every frame writes only transforms and opacity. */
  ...
  const ONLY='.workout-live-top,.workout-unconnected,.target-remaining,.music-tile,.music-thumb,.music-timeline,.music-heading [data-music="open"]';
  const DIAL='.timer-orbit,#timer-label,.timer-tap-hint';
  ...
  function hold(node,{rect:r,display},g){
    ...
    const parent=node.offsetParent||host(),origin=rect(parent,g)||{x:0,y:0};
  ...
  function begin(live){
    ...
    const m={live,start:focused,p:focused?1:0,v:0,target:focused,shared:[],leaving:[],held:[],cover,thumb,imgRect:img?rect(img,g):null,thumbRect:thumb?compact.only.get(thumb)?.rect:null,
      chevron:page.querySelector('#health-minimize'),dial:focused?[]:[...live.querySelectorAll(DIAL)],range:Math.max(200,Math.min(420,g.box.height/g.scale*.42))};
    m.flying=Boolean(m.imgRect&&m.thumbRect);
    for(const [node,c] of compact.shared){const f=focus.shared.get(node);if(f){Object.assign(node.style,{transformOrigin:'0 0',transition:'none'});m.shared.push({node,c,f,uniform:node.matches(UNIFORM),centred:node.id==='session-time'})}}
  ...
  function apply(){
    ...
    for(const s of m.shared){
      const at=m.start?s.f:s.c,v=between(s.c,s.f,p);
      if(s.centred){const k=v.w/at.w;s.node.style.transform=`translate(${v.x+v.w/2-at.x-at.w*k/2}px,${v.y+v.h/2-at.y-at.h*k/2}px) scale(${k})`;continue}
      const sy=v.h/at.h,sx=s.uniform?sy:v.w/at.w;s.node.style.transform=`translate(${v.x-at.x}px,${v.y-at.y}px) scale(${sx},${sy})`;
    }
    ...
    for(const node of m.dial)node.style.opacity=String(smooth(1-p,.45,1));
  ...
  function finish(){
    ...
    for(const s of m.shared){s.node.style.removeProperty('transform');s.node.style.removeProperty('transform-origin')}
    for(const node of m.leaving)node.style.removeProperty('opacity');
    for(const node of m.held)release(node);
    for(const node of m.dial)node.style.removeProperty('opacity');
    ...
    if(final!==m.start){
      // The reading has arrived where the other layout draws it; only the dial ring and labels are new.
      live.classList.toggle('timer-focused',final);attach(live,final);
      if(!final)for(const node of live.querySelectorAll(DIAL))animate(node,[{opacity:0},{opacity:1}],260);
    }
  ...
  function target(live,final){
    ...
    const dial=live.querySelector('.timer-dial');dial.setAttribute('aria-pressed',String(final));dial.setAttribute('aria-label',final?'Show workout details':'Focus on timer and music');live.querySelector('.timer-tap-hint').textContent=final?'Tap to show details':'Tap to focus';
...
const MusicPlayer=(()=>{
  ...
  function palette(img){
    try{
      ...
      for(let i=0;i<px.length;i+=4){
        const r=px[i]/255,g=px[i+1]/255,b=px[i+2]/255,[h,s,l]=hsl(r,g,b);
      ...
      const best=bins.reduce((a,b)=>b[3]>a[3]?b:a);if(best[3]<3)return null;
      ...
      return {accent:css(accent),'accent-2':css(second),ink:(lift+.05)/.057>1.05/(lift+.05)?'#17121b':'#ffffff',glass:`rgb(${shadow.map(v=>Math.round(v*255)).join(' ')} / .58)`,edge:`hsl(${degrees} 70% 72% / .45)`,soft:`hsl(${degrees} 55% 91%)`};
    }catch{return null}
  }
  function tint(value){const node=page();if(!value){node.classList.remove('has-art-palette');return}for(const [key,colour] of Object.entries(value))node.style.setProperty('--art-'+key,colour);node.classList.add('has-art-palette')}
  function clearArtwork(){artGeneration++;clearTimeout(artCleanup);artCleanup=0;cover?.remove();cover=null;art='';page().classList.remove('music-focus');backdrop('');tint(null);root?.querySelector('.music-thumb')?.querySelectorAll('i').forEach(node=>node.remove())}
  ...
  function retireArtwork(){
    ...
    page().classList.remove('music-focus');root?.classList.remove('has-art');backdrop('');tint(null);miniArt('');
  ...
  function artwork(value){
    ...
    img.decode().then(()=>{
      ...
      backdrop(small);host.classList.toggle('music-focus',focused);node.classList.add('has-art');miniArt(thumbnail(img,192)||value);tint(palette(img));
