...
internal class ExploreMotion(private val scope: CoroutineScope, initial: Boolean) {
    ...
    fun grab() { engage(); animation?.cancel(); dragging = true; phase = TraceGesture.DRAG }
    fun snap(open: Boolean) {
        animation?.cancel(); dragging = false; value = if (open) 1f else 0f; velocity = 0f
    ...
    fun settle(open: Boolean, speed: Float = velocity, reduced: Boolean = false) {
        ...
        animation = scope.launch {
            animate(value, if (open) 1f else 0f, initialVelocity = speed,
                animationSpec = spring(dampingRatio = 1f, stiffness = 289f, visibilityThreshold = .001f)) { pose, rate ->
