...
const WorkoutDetails=(()=>{
  ...
  function readings(s,activeMs){
    if(s.imported)return {distance:s.summary?.distance??null,average:null,calories:s.summary?.energy??null,max:null};
  ...
  const started=s=>new Date(s.startedAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
  const timeOnly=s=>!s.trackLocation&&!s.metrics?.points?.length;
  const note=(label,copy)=>`<div class="workout-stat workout-stat-note"><dt>${label}</dt><dd class="workout-stat-copy">${copy}</dd></div>`;
  // A session that records no location summarises what it did record: when it began, its pauses and its target.
  function compact(s,activeMs,totalMs=activeMs){const r=readings(s,activeMs);return `<dl class="workout-stat-grid">${timeOnly(s)?`${metric('Started',started(s))}${metric('Paused',clock(Math.max(0,totalMs-activeMs)))}${metric('Target',s.targetMs?s.targetMs/60000:'Open',s.targetMs?'min':'')}${r.calories===null?note('Energy estimate','Add your weight before a workout to see one.'):metric('Energy estimate',Math.round(r.calories),'kcal')}`:`${metric('Distance',r.distance===null?'—':(r.distance/1000).toFixed(2),'km')}${metric('Average pace',pace(r.average),'/km')}${metric('Average speed',r.average===null?'—':(r.average*3.6).toFixed(1),'km/h')}${metric('Energy estimate',r.calories===null?'—':Math.round(r.calories),'kcal')}`}</dl><div class="tracking-line"><span><i class="${s.metrics?.state==='tracking'?'tracking':''}"></i>${status(s)}</span><button data-workout-detail="active">All details <span>↗</span></button></div>${s.trackLocation&&['permission','unavailable','error'].includes(s.metrics?.state)&&window.OrbitWorkouts?.enableTracking?'<button class="notification-enable" data-tracking>Enable GPS tracking</button>':''}`}
  ...
  function chart(s,activeMs,mode){const points=s.metrics?.points||[];return `<div class="workout-chart-tabs glass-track blob-track" role="group" aria-label="Workout chart"><span class="selection-pill glass-indicator" aria-hidden="true"></span>${[['route','Route'],['speed','Speed'],['altitude','Elevation']].map(([key,label])=>`<button class="blob-option" data-workout-chart="${key}" aria-pressed="${mode===key}">${label}</button>`).join('')}</div><figure class="workout-record-chart">${mode==='route'?route(points):plot(points,mode,activeMs)}<figcaption>${mode==='route'?'Recorded on this phone · no street map':'Active time · gaps in GPS are left open'}</figcaption></figure>`}
  function actions(s){return `<div class="session-actions"><button data-session="${s.resumedAt===null?'resume':'pause'}">${s.resumedAt===null?'Resume workout':'Pause workout'}</button><button data-session="finish" aria-label="Finish workout">Finish</button></div>`}
  function view(s,activeMs,totalMs,mode='route'){
    if(s.imported)return importedView(s);
    ...
    return `<section class="workout-result"><p class="health-eyebrow">${active?'In progress':'Workout saved'} · ${date.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</p><div class="workout-result-value"><span role="img" aria-label="${r.distance===null?clock(activeMs)+' active time':(r.distance/1000).toFixed(2)+' kilometres'}" data-workout-hero="${r.distance===null?'time':'distance'}">${HeroDots.markup(r.distance!==null?(r.distance/1000).toFixed(2):clock(activeMs))}</span><span>${r.distance!==null?'km':'active time'}</span></div><p>${date.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}${s.endedAt?' — '+new Date(s.endedAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}):''} <span>· ${status(s)}</span></p></section>
      ...
      ${r.calories===null?'':`<section class="health-tile workout-energy"><div><h2>Energy estimate</h2><p class="workout-energy-number"><span>${Math.round(r.calories)}</span><small>kcal</small></p></div><details class="energy-method"><summary>How it’s estimated</summary><div class="details-body"><p>Based on ${s.weightKg} kg, active time and a general ${s.kind.toLowerCase()} intensity (${met[s.kind]} MET). Includes resting energy during active time. Actual energy use varies with effort.</p></div></details></section>`}<p class="health-note workout-source-note">${r.calories===null?'No energy estimate: a weight was not entered before this workout. ':''}${s.trackLocation?'Distance, pace and elevation use phone GPS. Poor or interrupted fixes are excluded. ':''}Only active time contributes to the energy estimate.</p>${active?actions(s):''}`;
  }
  function importedView(s){
    const v=s.summary||{},esc=HealthData.escape,number=HealthData.display,time=n=>new Date(n).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
    return `<section class="workout-result"><p class="health-eyebrow">Samsung Health · ${new Date(s.startedAt).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</p><div class="workout-result-value"><span role="img" aria-label="${clock(s.elapsed)} session duration">${HeroDots.markup(clock(s.elapsed))}</span><span>session duration</span></div><p>${time(s.startedAt)} — ${time(s.endedAt)}</p>${s.title?`<p>${esc(s.title)}</p>`:''}</section>
      <section class="health-tile workout-detail-group"><h2>Recorded during this session</h2><dl class="workout-stat-grid">${metric('Distance',number(v.distance==null?null:v.distance/1000,2),'km')}${metric('Active energy',number(v.energy),'kcal')}${metric('Steps',number(v.steps))}${metric('Average heart rate',number(v.heartAverage),'bpm')}${metric('Lowest heart rate',number(v.heartLow),'bpm')}${metric('Highest heart rate',number(v.heartHigh),'bpm')}</dl></section>
      ${s.laps?.length?`<section class="health-tile workout-detail-group"><h2>Laps</h2>${s.laps.map((lap,i)=>`<p>${i+1} · ${clock(lap.end-lap.start)}${lap.distanceM==null?'':' · '+number(lap.distanceM,0)+' m'}</p>`).join('')}</section>`:''}
