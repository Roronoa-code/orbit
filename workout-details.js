...
const WorkoutDetails=(()=>{
  ...
  function view(s,activeMs,totalMs,mode='route'){
    ...
    return `<section class="workout-result"><p class="health-eyebrow">${active?'In progress':'Workout saved'} · ${date.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}</p><div class="workout-result-value"><span role="img" aria-label="${r.distance===null?clock(activeMs)+' active time':(r.distance/1000).toFixed(2)+' kilometres'}" data-workout-hero="${r.distance===null?'time':'distance'}">${HeroDots.markup(r.distance!==null?(r.distance/1000).toFixed(2):clock(activeMs))}</span><span>${r.distance!==null?'km':'active time'}</span></div><p>${date.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}${s.endedAt?' — '+new Date(s.endedAt).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}):''} <span>· ${status(s)}</span></p>${active?`<div class="session-actions workout-live-actions"><button data-session="${s.resumedAt===null?'resume':'pause'}">${s.resumedAt===null?'Resume':'Pause'}</button><button data-session="finish">Finish</button></div>`:''}</section>
      ...
      ${r.calories===null?'':`<section class="health-tile workout-energy"><div><h2>Energy estimate</h2><p class="workout-energy-number"><span>${Math.round(r.calories)}</span><small>kcal</small></p></div><p>Based on ${s.weightKg} kg, active time and a general ${s.kind.toLowerCase()} intensity (${met[s.kind]} MET). Includes resting energy during active time. Actual energy use varies with effort.</p></section>`}<p class="health-note workout-source-note">${r.calories===null?'No energy estimate: a weight was not entered before this workout. ':''}${s.trackLocation?'Distance, pace and elevation use phone GPS. Poor or interrupted fixes are excluded. ':''}Only active time contributes to the energy estimate.</p>`;
  }
