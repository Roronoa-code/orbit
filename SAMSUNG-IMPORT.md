...
Follow-up: the earlier one-app check missed Samsung's Dual App profile (user 95). Its unlaunched Orbit copy has now been uninstalled; only Owner (user 0) has Orbit installed, and Secure Folder (user 150) does not. Future updates must use `install --user 0 -r` and verify all profile flags. The main APK and data were preserved. The connection remains Samsung Health → Health Connect → Orbit, with an exact Samsung origin filter; complete private Samsung history is not implied by access to shared records.
